    ...
    try:
        ...
        for category_name, tools_list in sorted(tools_by_category.items()):
            ...
            for tool_info in tools_list:
                tool_schemas.append(
                    SystemToolSchema(
                        name=tool_info["name"], tool_schema=tool_info["schema"]
                    )
                )
        ...
        return SystemToolListResponse(
            success=True, categories=categories, total_count=total_count
        )
...
async def get_system_tool_detail(
    tool_name: str, current_user: CurrentUser = Depends(get_current_user_hybrid)
):
    ...
    try:
        ...
        return SystemToolDetailResponse(
            success=True, name=tool_name, tool_schema=tool_schema, error=None
        )
