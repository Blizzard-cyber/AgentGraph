...
class MongoDBClient:
    ...
    async def initialize(self, connection_string: str, database_name: str = None):
        ...
        try:
            ...
            logger.info(
                f"MongoDB连接失败详细信息:{str(connection_string)}", exc_info=True
            )
            self.is_connected = False
            raise
    ...
    def _initialize_managers(self):
        ...
        self.graph_run_repository = GraphRunRepository(
            self.db, self.graph_run_messages_collection, self.conversation_repository
        )

        self.task_repository = TaskRepository(self.db)

        self.graph_config_repository = GraphConfigRepository(
            self.db, self.graphs_collection
        )

        self.prompt_repository = PromptRepository(self.db, self.prompts_collection)

        self.model_config_repository = ModelConfigRepository(
            self.db, self.model_configs_collection
        )

        self.mcp_config_repository = MCPConfigRepository(
            self.db, self.mcp_configs_collection
        )

        self.preview_repository = PreviewRepository(
            self.db, self.preview_shares_collection
        )

        self.user_repository = UserRepository(self.db, self.users_collection)

        self.invite_code_repository = InviteCodeRepository(
            self.db, self.invite_codes_collection
        )

        self.team_settings_repository = TeamSettingsRepository(
            self.db, self.team_settings_collection
        )

        self.refresh_token_repository = RefreshTokenRepository(
            self.db, self.refresh_tokens_collection
        )

        self.agent_repository = AgentRepository(self.db, self.agents_collection)

        self.agent_run_repository = AgentRunRepository(
            self.db, self.agent_run_collection
        )

        self.memory_repository = MemoryRepository(self.db, self.memories_collection)
    ...
    async def _create_indexes(self):
        ...
        try:
            ...
            await self.graph_run_messages_collection.create_index(
                [("conversation_id", 1)]
            )
            ...
            await self.tasks_collection.create_index(
                [("user_id", 1), ("task_name", 1), ("schedule_type", 1)], unique=True
            )
            await self.tasks_collection.create_index(
                [("execution_stats.last_executed_at.executed_at", -1)]
            )
            await self.tasks_collection.create_index(
                [("execution_stats.total_triggers", -1)]
            )

            await self.graphs_collection.create_index(
            ...
            await self.model_configs_collection.create_index(
                [("user_id", 1), ("name", 1)], unique=True
            )
            await self.model_configs_collection.create_index([("updated_at", -1)])

            await self.mcp_configs_collection.create_index(
            ...
            await self.preview_shares_collection.create_index(
                [("content_hash", 1)], unique=True
            )
            await self.preview_shares_collection.create_index(
            ...
            await self.refresh_tokens_collection.create_index(
                [("token_id", 1)], unique=True
            )
            ...
            await self.memories_collection.create_index(
                [("user_id", 1), ("owner_type", 1), ("owner_id", 1)], unique=True
            )
            await self.memories_collection.create_index([("user_id", 1)])
            await self.memories_collection.create_index([("updated_at", -1)])

            await self.conversation_shares_collection.create_index(
                [("share_id", 1)], unique=True
            )
            await self.conversation_shares_collection.create_index(
    ...
    async def get_conversation_with_messages(
        ...
        try:
            ...
            if conversation_type == "graph":
                # 图运行类型
                run_doc = await self.graph_run_repository.get_graph_run_messages_only(
                    conversation_id
                )
                ...
                # Agent 对话类型
                agent_run_doc = await self.agent_run_repository.get_agent_run(
                    conversation_id
                )
    ...
    async def list_conversations(
        ...
        return await self.conversation_repository.list_conversations(
            user_id, conversation_type, limit, skip
        )

    async def update_conversation_status(
        self, conversation_id: str, status: str, user_id: str = "default_user"
    ) -> bool:
        """更新对话状态"""
    ...
    async def update_conversation_title(
        self, conversation_id: str, title: str, user_id: str = "default_user"
    ) -> bool:
        """更新对话标题"""
    ...
    async def update_conversation_tags(
        self, conversation_id: str, tags: List[str], user_id: str = "default_user"
    ) -> bool:
        """更新对话标签"""
        return await self.conversation_repository.update_conversation_tags(
            conversation_id, tags, user_id
        )
    ...
    async def update_input_config(
        ...
        """更新对话的输入配置"""
        return await self.conversation_repository.update_input_config(
            conversation_id, input_config, user_id
        )

    async def update_conversation_title_and_tags(
        self, conversation_id: str, title: str = None, tags: List[str] = None
    ) -> bool:
        """更新对话标题和标签（首次生成时调用）"""
        return await self.conversation_repository.update_conversation_title_and_tags(
            conversation_id, title, tags
        )
    ...
    async def permanently_delete_conversation(self, conversation_id: str) -> bool:
        ...
        try:
            ...
            # 2. 删除 MinIO 中的所有文件
            from app.infrastructure.storage.object_storage.conversation_document_manager import (
                conversation_document_manager,
            )

            await conversation_document_manager.delete_all_conversation_files(
            ...
            if conversation_type == "graph":
                # 图运行类型，删除graph_run消息
                await self.graph_run_repository.delete_graph_run_messages(
                    conversation_id
                )
            elif conversation_type == "agent":
            ...
            # 4. 删除conversation元数据
            return await self.conversation_repository.permanently_delete_conversation(
                conversation_id
            )
    ...
    async def create_graph_config(
        ...
        """创建图配置"""
        return await self.graph_config_repository.create_graph(
            graph_name, graph_config, user_id
        )

    async def get_graph_config(
        self, graph_name: str, user_id: Optional[str] = None
    ) -> Optional[Dict[str, Any]]:
        """获取图配置"""
    ...
    async def update_graph_config(
        ...
        """更新图配置"""
        return await self.graph_config_repository.update_graph(
            graph_name, graph_config, user_id
        )
    ...
    async def rename_graph_config(
        self, old_name: str, new_name: str, user_id: Optional[str] = None
    ) -> bool:
        """重命名图"""
        return await self.graph_config_repository.rename_graph(
            old_name, new_name, user_id
        )

    async def graph_config_exists(
        self, graph_name: str, user_id: str = "default_user"
    ) -> bool:
        """检查图是否存在"""
        return await self.graph_config_repository.graph_exists(graph_name, user_id)

    async def add_graph_version_record(
        self,
        graph_name: str,
        ...
        """添加图配置版本记录"""
        return await self.graph_config_repository.add_version_record(
            graph_name, version_record, user_id
        )

    async def get_graph_version_info(
        self, graph_name: str, user_id: str = "default_user"
    ) -> Optional[Dict[str, Any]]:
        """获取图配置版本信息"""
        return await self.graph_config_repository.get_version_info(graph_name, user_id)

    async def remove_graph_version_record(
        self, graph_name: str, version_id: str, user_id: str = "default_user"
    ) -> bool:
        """移除图配置版本记录"""
        return await self.graph_config_repository.remove_version_record(
            graph_name, version_id, user_id
        )

    # === 预览短链管理 ===

    async def create_preview_share(
        self,
        lang: str,
        title: Optional[str],
        content: str,
        expire_hours: Optional[int] = 144,
        user_id: str = "default_user",
    ) -> Dict[str, Any]:
        """创建预览短链"""
        return await self.preview_repository.create_preview_share(
            lang, title, content, expire_hours, user_id
        )
    ...
    async def create_graph_run_conversation(
        self,
        conversation_id: str,
        graph_name: str,
        graph_config: Dict[str, Any],
    ...
    async def get_graph_run_conversation(
        self, conversation_id: str
    ) -> Optional[Dict[str, Any]]:
        """获取图运行对话"""
        return await self.graph_run_repository.get_graph_run_conversation(
            conversation_id
        )

    async def update_graph_run_data(
        self, conversation_id: str, update_data: Dict[str, Any]
    ) -> bool:
        """更新图运行数据"""
        return await self.graph_run_repository.update_graph_run_data(
            conversation_id, update_data
        )

    async def add_round_to_graph_run(
        self,
        conversation_id: str,
    ...
    async def update_graph_run_global_outputs(
        self, conversation_id: str, node_name: str, output: str
    ) -> bool:
        """更新图运行全局输出"""
        return await self.graph_run_repository.update_global_outputs(
            conversation_id, node_name, output
        )

    async def update_graph_run_handoffs_status(
        self, conversation_id: str, node_name: str, handoffs_data: Dict[str, Any]
    ) -> bool:
        """更新图运行handoffs状态"""
        return await self.graph_run_repository.update_handoffs_status(
            conversation_id, node_name, handoffs_data
        )

    async def update_graph_run_execution_chain(
        self, conversation_id: str, execution_chain: List[Any]
    ) -> bool:
        """更新图运行执行链"""
        return await self.graph_run_repository.update_execution_chain(
            conversation_id, execution_chain
        )

    async def update_graph_run_final_result(
        self, conversation_id: str, final_result: str
    ) -> bool:
        """更新图运行最终结果"""
        return await self.graph_run_repository.update_final_result(
            conversation_id, final_result
        )
    ...
    async def update_mcp_config(
        self, config: Dict[str, Any], expected_version: int
    ) -> Dict[str, Any]:
        """更新团队MCP配置"""
        return await self.mcp_config_repository.update_mcp_config(
            config, expected_version
        )

    async def initialize_mcp_config(
        self, initial_config: Dict[str, Any] = None
    ) -> bool:
        """初始化团队MCP配置"""
    ...
    async def check_server_provider(self, server_name: str, user_id: str) -> bool:
        """检查用户是否是MCP服务器的提供者"""
        return await self.mcp_config_repository.check_server_provider(
            server_name, user_id
        )
    ...
    async def get_conversation_share_by_conversation(
        self, conversation_id: str
    ) -> Optional[Dict[str, Any]]:
        """根据对话ID获取分享记录"""
    ...
    async def delete_conversation_shares_by_conversation(
        self, conversation_id: str
    ) -> bool:
        """删除对话的所有分享记录（级联删除）"""
        return await self.share_repository.delete_shares_by_conversation(
            conversation_id
        )
