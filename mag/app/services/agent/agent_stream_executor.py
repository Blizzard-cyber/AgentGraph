...
class AgentStreamExecutor:
    ...
    async def run_agent_stream(
        ...
        try:
            ...
            if final_result:
                ...
                memory_info = await self.extract_memory_info(final_result.get("round_messages", []))
                try:
                    ...
                    if memory_result.get("success"):
                        logger.info(f"记忆保存成功: user_id={user_id}, agent_id={effective_config['agent_name']}, session_id={conversation_id}")
                    else:
                        logger.warning(f"记忆保存失败: {memory_result.get('error', '未知错误')}")
    ...
    async def extract_memory_info(self, messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
