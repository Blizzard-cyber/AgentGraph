...
class ConversationListItem(BaseModel):
    ...
    status: str = Field(
        ..., description="对话状态：active（活跃）/ deleted（已删除）/ favorite（收藏）"
    )
...
class InputConfig(BaseModel):
    ...
    selected_mcp_servers: Optional[List[str]] = Field(
        None, description="选择的 MCP 服务器列表"
    )
    selected_agent: Optional[str] = Field(None, description="选择的 Agent")
    selected_system_tools: Optional[List[str]] = Field(
        None, description="选择的系统工具"
    )
...
class ConversationDetailResponse(BaseModel):
    ...
    rounds: List[Dict[str, Any]] = Field(
        default_factory=list, description="完整消息轮次（原始格式）"
    )
    type: Optional[str] = Field(
        None, description="对话类型：agent（Agent对话）/ graph（图执行）"
    )

    # 文档/文件信息
    documents: Optional[Dict[str, Any]] = Field(
        None, description="对话关联的文档/文件信息"
    )

    # AI生成对话的解析结果（图生成/MCP生成时）
    parsed_results: Optional[Dict[str, Any]] = Field(
        None, description="AI生成的解析结果"
    )
    ...
    tasks: Optional[List[Dict[str, Any]]] = Field(
        None, description="Sub Agent 任务列表"
    )
...
class ConversationCompactRequest(BaseModel):
    ...
    compact_type: str = Field(
        default="brutal",
        description="压缩类型：precise（精确压缩）/ brutal（暴力压缩）",
    )
    compact_threshold: int = Field(
        default=2000, description="压缩阈值，超过此长度的tool content将被压缩"
    )
    user_id: str = Field(default="default_user", description="用户ID")
...
class UpdateConversationStatusRequest(BaseModel):
    ...
    status: str = Field(
        ..., description="新状态：active（活跃）/ deleted（软删除）/ favorite（收藏）"
    )
    user_id: str = Field(default="default_user", description="用户ID")
